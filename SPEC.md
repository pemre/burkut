...
- The `useMdLoader` hook imports all content from a single virtual module (`virtual:md-content`) generated by a custom Vite plugin (`vite-plugins/md-content.ts`). The plugin scans all `.md` files under `src/content/`, parses them with `gray-matter` in Node.js at build/dev time, and emits one JS module containing every file's `{ data, content }`. The browser never loads `gray-matter` or `Buffer`, and in dev mode there is a single module request instead of ~75 individual `.md` fetches. `getContent(id)` is synchronous — no runtime network requests, instant first render. Header files are detected by path depth (direct children of `content/` = headers).
...
| `useMdLoader` | Eagerly indexes all pre-parsed .md modules at build time, provides synchronous getContent(id) |
...
- `src/components/ContentPanel/ContentPanel.test.tsx` – synchronous content rendering, fallback
...
- `import.meta.glob({ eager: true })` embeds all `.md` content into the JS bundle at build time — zero runtime network requests, instant first render. Trade-off: ~100-150KB added to the bundle (pre-gzip), but gzip compresses text ~70-80%, and this is far better than ~60 HTTP roundtrips on GitHub Pages.
- `.md` files are pre-parsed at build/dev time by the custom Vite plugin `vite-plugins/md-content.ts`. The plugin scans all `.md` files, runs `gray-matter` in Node.js (where `Buffer` is native), and serves them as a single virtual module (`virtual:md-content`). This eliminates `gray-matter` and `Buffer` from the browser bundle entirely (~80 KB savings), removes `eval` warnings, and reduces dev-mode HTTP requests from ~75 to 1. The plugin watches the content directory for HMR — editing a `.md` file triggers a full reload.
